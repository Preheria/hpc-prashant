...
int main() {
	...
	if (size != 9){
      if (rank == 0){
         printf("This program needs exactly 9 processes to run.\n");
         return 0;
      }
   }
  	while(!minimum_found) {
		 if (rank != 0){
				i = rank - 1;//(0-8 size of array))
				printf("rank : %d \n",rank);
		 		dm[i] = bm + (om[i] * step);
		  		dc[i] = bc + (oc[i] * step);
				...
				MPI_Send (&e[i], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);

				MPI_Recv (&bm, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				MPI_Recv (&bc, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				MPI_Recv (&minimum_found, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			for(i=0;i<(size-1);i++) {
				...
				MPI_Recv (&e[i], 1, MPI_DOUBLE, i+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		  		if(e[i] < best_error) {
		    		best_error = e[i];
		    		best_error_i = i;
		...
		for(i=0;i<(size-1);i++) {
			MPI_Send (&bm, 1, MPI_DOUBLE, i+1, 1, MPI_COMM_WORLD);
			MPI_Send (&bc, 1, MPI_DOUBLE, i+1, 1, MPI_COMM_WORLD);
			MPI_Send (&minimum_found, 1, MPI_DOUBLE, i+1, 1, MPI_COMM_WORLD);
		}
  	}
}
	if(rank==0){
